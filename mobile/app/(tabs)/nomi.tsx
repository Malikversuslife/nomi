...
const starters = ["Explain Factorisation another way","Show me a worked example","Quiz me on the step I keep missing"];
const tutorApiBase = process.env.EXPO_PUBLIC_NOMI_API_URL?.replace(/\/$/, "") ?? "";

function tutorReply(message:string, mastery:number, difficulty:number, misconception:ReturnType<typeof usePracticeProgress>["misconception"]):TutorResult{
  ...
  if(text.includes("worked example")||text.includes("example"))return{content:`${context}\n\nTry 18x + 30. The greatest common factor is 6, so 18x + 30 = 6(3x + 5).\n\nThe key is to factor out the greatest common factor, not just any shared factor.`,followUp:"Want to try one yourself?",suggestedAction:"practice"};
  if(text.includes("quiz")||text.includes("question")||text.includes("test"))return{content:`${context}\n\nQuick check: factorise 24x + 36 completely.`,followUp:"What is the greatest common factor of 24 and 36?",suggestedAction:"none"};
  return{content:`${context}\n\nThink of Factorisation as packing terms into equal groups. In 12x + 16, both terms can be packed into groups of 4, so 12x + 16 = 4(3x + 4).`,followUp:"Want an example or a quick check?",suggestedAction:"example"};
}
...
export default function NomiScreen(){
  ...
  async function getTutorResponse(message:string,currentMessages:TutorMessage[]):Promise<TutorResult>{
    setTutorError(null);
    if(!tutorApiBase){setAiMode("error");setTutorError("Tutor API URL is not configured.");return tutorReply(message,mastery,adaptivePractice.difficulty,misconception);}
    if(!supabase){setAiMode("error");setTutorError("Supabase client is not configured.");return tutorReply(message,mastery,adaptivePractice.difficulty,misconception);}
    const{data,error:sessionError}=await supabase.auth.getSession();
    const accessToken=data.session?.access_token;
    if(sessionError||!accessToken){setAiMode("error");setTutorError(sessionError?.message??"No authenticated tutor session is available.");return tutorReply(message,mastery,adaptivePractice.difficulty,misconception);}
    try{
      ...
      try{payload=raw?JSON.parse(raw):{};}catch{setAiMode("error");setTutorError(`Tutor API returned invalid JSON (${response.status}).`);return tutorReply(message,mastery,adaptivePractice.difficulty,misconception);}
      ...
      setAiMode("error");setTutorError(payload.error??payload.message??`Tutor API request failed with status ${response.status}.`);
    }catch(error){setAiMode("error");setTutorError(error instanceof Error?error.message:"Tutor API network request failed.");}
