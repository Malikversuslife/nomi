...
export default function NomiScreen(){
  ...
  const{topics,currentTopic,isPathComplete,parentName}=useCurriculum("mathematics");
  const{mastery,misconception,adaptivePractice,latestSession,activeTopicName,setActiveTopic}=usePracticeProgress();
  const reviewTopic=topics.at(-1)??null;

  useEffect(()=>{
    if(currentTopic)setActiveTopic(currentTopic.id,currentTopic.name);
  },[currentTopic,setActiveTopic]);

  const firstName=displayName?.split(" ")[0]??"there";
  const topicName=currentTopic?.name??activeTopicName??latestSession?.topic??reviewTopic?.name??"Factorisation";
  const displayMastery=currentTopic?.mastery??reviewTopic?.mastery??mastery;
  const displayDifficulty=currentTopic?.difficulty??reviewTopic?.difficulty??adaptivePractice.difficulty;
  const starters=useMemo(()=>isPathComplete?["What should I learn next?","Give me a mixed review of quadratic equations","Which concept should I revisit first?"]:[`Explain ${topicName} another way`,"Show me a worked example","Quiz me on the step I keep missing"],[isPathComplete,topicName]);
  const[input,setInput]=useState("");
  const[messages,setMessages]=useState<TutorMessage[]>([]);
  const[sending,setSending]=useState(false);
  const[aiMode,setAiMode]=useState<"server"|"fallback"|"error"|null>(null);
  const[tutorError,setTutorError]=useState<string|null>(null);
  const contextLine=useMemo(()=>isPathComplete?`${parentName??"Quadratic equations"} · path complete · ${topics.length} concepts mastered`:`${topicName} · ${displayMastery}/100 mastery · difficulty ${displayDifficulty}${misconception?` · ${misconception.status} misconception`:" · no active misconception"}`,[isPathComplete,parentName,topics.length,topicName,displayMastery,displayDifficulty,misconception]);

  async function getTutorResponse(message:string,currentMessages:TutorMessage[]):Promise<TutorResult>{
    setTutorError(null);
    if(isPathComplete){
      const text=message.toLowerCase();
      if(text.includes("next"))return{content:"You have completed the Quadratic equations path. The best next move is to build forward into the next Algebra unit when it is available, while using mixed review to keep these three concepts durable.",followUp:"Want a mixed review before moving on?",suggestedAction:"review"};
      if(text.includes("review")||text.includes("revisit"))return{content:"Your Quadratic equations path is complete. I can mix Factorisation, Completing the square and Quadratic formula so review is based on the full path instead of pretending one topic is still current.",followUp:"Start with a mixed review plan?",suggestedAction:"none"};
    }
    if(!tutorApiBase){setAiMode("fallback");setTutorError("Tutor API URL is not configured. Using Nomi's deterministic local fallback.");return tutorReply(message,topicName,displayMastery,displayDifficulty,misconception);}
    if(!supabase){setAiMode("fallback");setTutorError("Supabase client is not configured. Using Nomi's deterministic local fallback.");return tutorReply(message,topicName,displayMastery,displayDifficulty,misconception);}
    const{data,error:sessionError}=await supabase.auth.getSession();
    const accessToken=data.session?.access_token;
    if(sessionError||!accessToken){setAiMode("fallback");setTutorError(sessionError?.message??"No authenticated tutor session is available. Using the local fallback.");return tutorReply(message,topicName,displayMastery,displayDifficulty,misconception);}
    try{
      ...
      const response=await fetch(`${tutorApiBase}/api/mobile/tutor`,{method:"POST",headers:{authorization:`Bearer ${accessToken}`,"content-type":"application/json"},body:JSON.stringify({message,transcript,topicId:currentTopic?.id??null})});
      const raw=await response.text();
      let payload:{ok?:boolean;message?:string;followUp?:string|null;suggestedAction?:TutorAction;error?:string}={};
      try{payload=raw?JSON.parse(raw):{};}catch{setAiMode("fallback");setTutorError(`Tutor API returned invalid JSON (${response.status}). Using the local fallback.`);return tutorReply(message,topicName,displayMastery,displayDifficulty,misconception);}
    ...
    return tutorReply(message,topicName,displayMastery,displayDifficulty,misconception);
  }
  ...
  function actionLabel(action:TutorAction){if(action==="practice")return"Try assessed Practice";if(action==="review")return"Review the path";if(action==="example")return"Show me an example";return null;}

  return <SafeAreaView style={styles.safeArea} edges={["top","left","right"]}><ScrollView contentContainerStyle={styles.screen} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
    <Text style={styles.eyebrow}>NOMI</Text><Text style={styles.title}>{isPathComplete?`You finished this path, ${firstName}. What should we do next?`:`What should we work through, ${firstName}?`}</Text><Text style={styles.description}>{isPathComplete?"I’m treating Quadratic equations as complete, so our conversation can focus on review, transfer and what comes next.":"I’m carrying your active topic and learner evidence into this conversation, so you don’t have to explain where you are every time."}</Text>
    <View style={styles.contextCard}><Text style={styles.cardEyebrow}>LEARNER CONTEXT</Text><Text style={styles.contextText}>{contextLine}</Text>{latestSession?<Text style={styles.contextMeta}>Latest practice: {latestSession.topic} · {latestSession.score}/{latestSession.total} correct · {latestSession.accuracy}% accuracy</Text>:null}<Text style={styles.modeNote}>{aiMode==="server"?"AI tutor connected":aiMode==="fallback"?"Using local tutor fallback":aiMode==="error"?"Tutor connection needs attention":"Tutor ready"}</Text>{tutorError?<Text style={styles.errorNote}>{tutorError}</Text>:null}</View>
